...
class VaryingParameter(BaseModel):
    ...
    min_value: float = Field(..., description="Minimum value of the parameter")
    max_value: float = Field(..., description="Maximum value of the parameter")
    step_size: float = Field(..., gt=0, description="Step size for parameter range")
...
def create_parameter_array(varying_param: VaryingParameter) -> np.ndarray:
    ...
    return np.arange(
        varying_param.min_value,
        varying_param.max_value + varying_param.step_size,
        varying_param.step_size,
    )


def compute_all_functions_single(
    params: ReservoirParameters, time_array: np.ndarray, include_gradient: bool = True
) -> pd.DataFrame:
    ...
    for td in time_array:
        ...
        for param_value in param_array:
            ...
            try:
                ...
                st.error(
                    f"Error computing values at tD = {td}, {varying_param.parameter_name} = {param_value}: {str(e)}"
                )
                continue
...
def create_single_parameter_individual_plots(
    df: pd.DataFrame,
    ...
    num_plots = (
        3 if include_gradient and "Pressure Gradient (∂pD/∂tD)" in df.columns else 2
    )
    ...
    fig.suptitle(
        f"Water Breakthrough Analysis - {title_suffix}", fontsize=16, fontweight="bold"
    )
    ...
    axes[0].set_xlabel(
        "Dimensionless Time (tD) - Log Scale", fontsize=12, fontweight="bold"
    )
    axes[0].set_ylabel("Dimensionless Pressure (pD)", fontsize=12)
    axes[0].set_title(
        "Dimensionless Pressure vs Log(tD)", fontsize=13, fontweight="bold"
    )
    axes[0].grid(
        show_grid, alpha=0.3, which="both"
    ...
    if num_plots == 3 and "Pressure Gradient (∂pD/∂tD)" in df.columns:
        ...
        axes[2].semilogx(
            time_data,
            df["Pressure Gradient (∂pD/∂tD)"],
            "g-",
            linewidth=line_width,
            marker=marker_style3,
            markersize=marker_size,
        )
        axes[2].set_xlabel(
            "Dimensionless Time (tD) - Log Scale", fontsize=12, fontweight="bold"
        ...
        axes[2].set_title(
            "Pressure Gradient vs Log(tD)", fontsize=13, fontweight="bold"
        )
        axes[2].grid(
            show_grid, alpha=0.3, which="both"
        )  # Show both major and minor grid lines
...
def create_varying_parameter_individual_plots(
    df: pd.DataFrame,
    varying_param: VaryingParameter,
    show_markers: bool = True,
    show_grid: bool = True,
    ...
    # Determine number of subplots based on available data
    has_gradient = include_gradient and any(
        col.startswith("∂pD/∂tD") for col in df.columns
    )
    ...
    axes[0].xaxis.set_major_formatter(FuncFormatter(format_func))
    axes[0].legend(
        title=varying_param.parameter_name.replace("_", " ").title(),
...
def create_combined_plot(
    df: pd.DataFrame,
    plot_type: typing.Literal["separate_y", "combined"] = "separate_y",
    show_markers: bool = True,
    show_grid: bool = True,
    line_width: float = 2.5,
    include_gradient: bool = True,
    ...
    if plot_type == "separate_y":
        ...
        plt.title(
            f"Water Breakthrough Analysis - Semilog Plot ({params_count})",
            fontsize=14,
            fontweight="bold",
            pad=20,
        )
        ...
        pressure_norm = (
            df["Dimensionless Pressure (pD)"] - df["Dimensionless Pressure (pD)"].min()
        ) / (
            df["Dimensionless Pressure (pD)"].max()
            - df["Dimensionless Pressure (pD)"].min()
        )
        derivative_norm = (
            df["Pressure Derivative (p'D)"] - df["Pressure Derivative (p'D)"].min()
        ) / (
            df["Pressure Derivative (p'D)"].max()
            - df["Pressure Derivative (p'D)"].min()
        )
        ...
        if has_gradient:
            gradient_norm = (
                df["Pressure Gradient (∂pD/∂tD)"]
                - df["Pressure Gradient (∂pD/∂tD)"].min()
            ) / (
                df["Pressure Gradient (∂pD/∂tD)"].max()
                - df["Pressure Gradient (∂pD/∂tD)"].min()
            )
            marker3 = "^" if show_markers else None
            ax.semilogx(
        ...
        params_count = "3 Parameters" if has_gradient else "2 Parameters"
        ax.set_title(
            f"Water Breakthrough Analysis - Semilog Plot (Normalized, {params_count})",
            fontsize=14,
            fontweight="bold",
        )
        ax.legend(loc="best")
...
def create_separate_tables(
    df: pd.DataFrame, varying_param: VaryingParameter, include_gradient: bool = True
):
...
def save_plot(fig: plt.Figure, filename: str, dpi: int = 300) -> BytesIO:
    """
    Save plot as image in memory.

    :param fig: Matplotlib figure object
    :param filename: Filename for saving (not used in memory saving)
...
def main():
    ...
    st.header("Water Breakthrough Analysis")
    st.markdown("""
    ...
    **📋 Quick Guide:**
    ...
    st.markdown(
        "*Configure the horizontal well and reservoir properties (these remain constant)*"
    )
    ...
    st.markdown("#### 🔄 Parameter Variation")
    st.markdown(
        "*Choose whether to use single values or vary one parameter for comprehensive analysis*"
    )
    ...
    if use_parameter_variation:
        st.info(
            "🔄 **Parameter Variation Mode**: One parameter will be varied while others remain constant."
        )
        ...
        st.info(
            "📊 **Single Value Mode**: All parameters will use their base values for calculations."
        )
    ...
    with param_col1:
        ...
        if not (
            use_parameter_variation and selected_param_key == "dimensionless_length"
        ):
            ...
            st.info(
                "🔄 **Dimensionless Length (LD)** is set to vary. Configure above in Parameter Variation section"
            )

        # Dimensionless Wellbore Radius input
        if not (
            use_parameter_variation
            and selected_param_key == "dimensionless_wellbore_radius"
        ):
            ...
            st.info(
                "🔄 **Dimensionless Wellbore Radius (r_wD)** is set to vary. Configure above in Parameter Variation section"
            )
    ...
    with param_col2:
        ...
        if not (
            use_parameter_variation
            and selected_param_key == "wellbore_storage_constant"
        ):
            ...
            st.info(
                "🔄 **Wellbore Storage Constant (CD)** is set to vary. Configure above in Parameter Variation section"
            )

        # Distance to Boundary input
        if not (
            use_parameter_variation and selected_param_key == "distance_to_boundary"
        ):
        ...
        if not (use_parameter_variation and selected_param_key == "skin_factor"):
            ...
            st.info(
                "🔄 **Skin Factor (S)** is set to vary. Configure above in Parameter Variation section"
            )
    ...
    if st.button(
        ...
        try:
            ...
            if use_parameter_variation:
                ...
                with st.spinner(
                    "Computing water breakthrough analysis with parameter variation..."
                ):
                ...
                with st.spinner(
                    "Computing water breakthrough analysis with single parameter values..."
                ):
    ...
    if (
        st.session_state.get("computation_done", False)
        ...
        if use_parameter_variation and varying_param:
            ...
            st.markdown(
                f"*Computed values with varying {varying_param_name} - Separate tables for each pressure type*"
            )

            # Create separate tables
            tables = create_separate_tables(
                df=results_df,
                varying_param=varying_param,
            ...
            if len(table_names) > 1:
                ...
                for i, (table_name, table_df) in enumerate(tables.items()):
                    with tabs[i]:
                        ...
                        st.dataframe(formatted_table, width="stretch", height="auto")
            else:
                # Single table
            ...
            st.markdown(
                f"*Computed values for {', '.join(pressure_types)} using single parameter values*"
            )
            ...
            st.dataframe(formatted_df, width="stretch", height="auto")
        ...
        st.markdown(
            "*Interactive plots for breakthrough timing analysis and interpretation*"
        )
        ...
        with plot_col1:
            ...
            plot_option = st.selectbox(
                ...
                [
                    "Individual Plots",
                    "Combined (Separate Y-Axes)",
                    "Combined (Normalized)",
                ],
                help="Select how you want to visualize the data",
            )

        with plot_col2:
            st.markdown("**💾 Download Options:**")
            if st.button(
                "🖼️ Generate & Download Plots",
                help="Create high-quality plots for download",
            ):
                with st.spinner("Generating plots..."):
                    if plot_option == "Individual Plots":
                        if use_parameter_variation and varying_param:
                            fig, _ = create_varying_parameter_individual_plots(
                                df=results_df,
                                varying_param=varying_param,
                            ...
                            img_buffer = save_plot(
                                fig=fig,
                                filename="individual_plots_varying.png",
                                dpi=plot_dpi,
                            )
                            st.download_button(
                                label="📥 Download Individual Plots",
                                data=img_buffer.getvalue(),
                                file_name=f"individual_plots_varying_{varying_param.parameter_name}.png",
                                mime="image/png",
                            )
                        else:
                            fig, _ = create_single_parameter_individual_plots(
                                df=results_df,
                                show_markers=show_markers,
                                show_grid=show_grid,
                                line_width=line_width,
                            ...
                            img_buffer = save_plot(
                                fig=fig,
                                filename="individual_plots_single.png",
                                dpi=plot_dpi,
                            )
                            st.download_button(
                                label="📥 Download Individual Plots",
                                data=img_buffer.getvalue(),
                                file_name="individual_plots.png",
                                mime="image/png",
                            )
                        ...
                        if not use_parameter_variation:
                            ...
                            img_buffer = save_plot(
                                fig=fig,
                                filename="combined_separate_y.png",
                                dpi=plot_dpi,
                            )
                            st.download_button(
                            ...
                            st.info(
                                "Combined plots are only available for single parameter mode."
                            )
                    else:  # Normalized
                        if not use_parameter_variation:
                            fig = create_combined_plot(
                            ...
                            img_buffer = save_plot(
                                fig=fig,
                                filename="combined_normalized.png",
                                dpi=plot_dpi,
                            )
                            st.download_button(
                                label="📥 Download Normalized Plot",
                                data=img_buffer.getvalue(),
                                file_name="normalized_plot.png",
                                mime="image/png",
                            )
                        else:
                            st.info(
        ...
        try:
            ...
            if results_df["Dimensionless Time (tD)"].min() <= 0:
                st.warning(
                    "⚠️ Warning: Some time values are zero or negative. This may cause plotting issues."
                )
            ...
            if plot_option == "Individual Plots":
                ...
                if use_parameter_variation and varying_param:
                    varying_param_name = varying_param.parameter_name.replace(
                        "_", " "
                    ).title()
                    st.markdown(
                        f"**📈 Individual Parameter Plots (Semilog - {param_count}):**"
                    )
                    st.info(
                        f"Each parameter plotted on semilog scale with multiple lines for varying {varying_param_name}. Logarithmic time axis for detailed analysis of water breakthrough behavior across time decades."
                    )
                    fig, _ = create_varying_parameter_individual_plots(
                        df=results_df,
                        varying_param=varying_param,
                    ...
                    st.info(
                        f"Each parameter plotted on semilog scale with tD axis logarithmic for detailed analysis of water breakthrough behavior across time decades. Currently showing {param_count}."
                    )
                    fig, _ = create_single_parameter_individual_plots(
                        df=results_df,
                        show_markers=show_markers,
                ...
                if use_parameter_variation:
                    st.info(
                        "Combined plots are only available for single parameter mode. Please use Individual Plots for parameter variation analysis."
                    )
                else:
                    param_count = "3 parameters" if include_gradient else "2 parameters"
                    st.markdown(
                        f"**📊 Combined Water Breakthrough Semilog Plot with Separate Y-Axes ({param_count}):**"
                    )
                    st.info(
                        f"All {param_count} on one semilog plot with separate y-axes to analyze water breakthrough patterns across different scales with logarithmic time axis."
                    )
                    fig = create_combined_plot(
                        df=results_df,
                        plot_type="separate_y",
                ...
                if use_parameter_variation:
                    ...
                    param_count = "3 parameters" if include_gradient else "2 parameters"
                    st.markdown(
                        f"**🔄 Combined Normalized Water Breakthrough Semilog Plot ({param_count}):**"
                    )
                    st.info(
                        f"All {param_count} normalized to 0-1 range on semilog scale for direct comparison of water breakthrough trends with logarithmic time axis."
                    )
                    fig = create_combined_plot(
                        df=results_df,
                        plot_type="normalized",
        ...
        with st.expander("📖 Plot Interpretation Guide", expanded=False):
            st.markdown("""
            ### How to Interpret the Semilog Plots:
            ...
            **🌊 Key Observations on Semilog Scale:**
        ...
        if use_parameter_variation and varying_param:
            st.markdown(
                "*Key metrics from the computed analysis across parameter variations*"
            )
            ...
            if len(param_values) > 0:
                ...
                st.markdown(
                    f"**Summary across {len(param_values)} {varying_param_name} values:**"
                )

                # Create columns for each parameter value (limit to 4 for display)
                display_params = (
                    param_values[:4] if len(param_values) > 4 else param_values
                )
                ...
                for i, param_val in enumerate(display_params):
                    with summary_cols[i]:
                        ...
                        if pd_col in results_df.columns:
                            ...
                            max_pd_time = results_df.loc[
                                results_df[pd_col].idxmax(), "Dimensionless Time (tD)"
                            ]
                            st.metric(
                                "Max pD",
                                f"{max_pd:.4f}",
                        ...
                        pd_deriv_col = (
                            f"p'D ({varying_param.parameter_name}={param_val:.3f})"
                        )
                        if pd_deriv_col in results_df.columns:
                            max_pd_deriv = results_df[pd_deriv_col].max()
                            max_pd_deriv_time = results_df.loc[
                                results_df[pd_deriv_col].idxmax(),
                ...
                if len(param_values) > 4:
                    st.info(
                        f"Showing summary for first 4 parameter values. Total computed: {len(param_values)} values."
                    )
        else:
            st.markdown("*Key metrics from the computed analysis*")
        ...
        if st.button(
            ...
            help="Clear current results to start a new analysis",
        ):
        ...
        st.info(
            "👆 Enter parameters above and click 'Start Analysis' to see results and visualizations."
        )
